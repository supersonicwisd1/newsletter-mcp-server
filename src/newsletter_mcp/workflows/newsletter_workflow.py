...
class NewsletterWorkflow:
    ...
    def __init__(self, slack_token: str):
        ...
        print("✅ Newsletter workflow initialized with Slack, Google Docs, and Gmail tools")
    ...
    async def generate_newsletter(
        ...
        if send_email:
            try:
                ...
                print(f"✅ Email sent successfully to {len(custom_recipients) if custom_recipients else 'subscribers'}")
    ...
    async def _send_newsletter_email(
        ...
        try:
            ...
            if custom_recipients:
                ...
                print(f"📧 Using custom recipients: {len(recipients)} addresses")
            else:
                # Get recipients from environment
                recipients_str = os.getenv('NEWSLETTER_SUBSCRIBER_EMAILS', '')
                if not recipients_str:
                    print("⚠️  No newsletter subscribers configured (NEWSLETTER_SUBSCRIBER_EMAILS)")
                    return None
                ...
                print(f"📧 Using environment recipients: {len(recipients)} addresses")
            
            if not recipients:
                print("⚠️  No valid email recipients found")
                return None
            ...
            print(f"✅ Newsletter email sent to {len(recipients)} recipients")
            return result
